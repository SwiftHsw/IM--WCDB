...
- (void) didSelectedFace:(UIButton *)sender
{
    if (sender.tag == -1) {
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(chatBoxFaceViewDidSelectedFace:groupId:type:)]) {
            
            [_delegate chatBoxFaceViewDidSelectedFace:face groupId:_curGroup.groupID type:_curGroup.faceType];
        }
    }
}
...
- (void) reloadScrollView
{
     ...
     */
    int page;
    int c_page1 = (int)(self.curGroup.facesArray.count / (self.curGroup.faceType == TLFaceTypeEmoji ? 20 : 8));
    int remainder_num = (int)(self.curGroup.facesArray.count % (self.curGroup.faceType == TLFaceTypeEmoji ? 20 : 8));
    if (remainder_num > 0) {
        page = c_page1 + 1;
    }else{
        page = c_page1;
...
- (ZXChatFaceMenuView *) faceMenuView
{
    ...
    if (_faceMenuView == nil) {
        _faceMenuView = [[ZXChatFaceMenuView alloc] initWithFrame:CGRectMake(0, self.height - HEIGHT_BOTTOM_VIEW, SCREEN_WIDTH, HEIGHT_BOTTOM_VIEW)];
